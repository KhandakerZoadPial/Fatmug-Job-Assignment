...
def update_vendor_performance(sender, instance, created, **kwargs):
    if created or instance.status == 'completed' or instance.quality_rating or instance.acknowledgment_date:
        ...
        total_completed_orders = PurchaseOrder.objects.filter(vendor=vendor).count()
        successful_orders = PurchaseOrder.objects.filter(vendor=vendor, status='completed').count()
        fulfillment_rate = successful_orders / total_completed_orders if total_completed_orders > 0 else 0
        ...
        if created:
            ...
            v = VendorPerformance.objects.get(vendor=vendor)
            v.date = timezone.now()
            v.on_time_delivery_rate = on_time_delivery_rate
            v.quality_rating_avg = quality_rating_avg
            v.average_response_time = average_response_time.total_seconds() / 60 if average_response_time else 0
            v.fulfillment_rate = fulfillment_rate
            v.save()
